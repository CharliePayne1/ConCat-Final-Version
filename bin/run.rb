...
#input new user name
new_user_name = prompt.ask("To get started, tell us your name:".colorize(:red))
...
active_projects = prompt.yes?("Would you like to look at your active projects?")
    if active_projects
        project_names = new_user.projects.map {|project| project.name}
            ...
            puts "You don't have any active projects currently, let's change that!."
            else
...
new_app_name = prompt.ask("If you'd like to start working with #{developer.name}, tell us the name of your new app:".colorize(:green))
...
puts "Okay great, we've created a new project for you and #{developer.name} to start working on #{new_app_name}.".colorize(:green)
puts " "
...
update_answer = prompt.yes?('Would you like to change the name of your app?')
        if update_answer
            original_app_name = prompt.ask("To help us find your app, what did you call it originally?".colorize(:green))
            original_app = Project.find_by(name: "#{original_app_name}")

            updated_app_name = prompt.ask('Found it! Now, what would you like to change the name to?'.colorize(:red))
...
if delete_project
    app_name = prompt.ask("To help us find your app, what did you call your it?".colorize(:red))
